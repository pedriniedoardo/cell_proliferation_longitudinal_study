...
#analyze the data
  ...
  #try to remove the well that died in the early phase and well 235 (strange behaviuor)
  #df6<-df5
  #die<-c(240,239,236,234,201,186,183,182,179,177,176,170,167,162,235)
  #mat_die<-sapply(die,FUN = function(x){df6$well_ID==x})
  #index_not_die<-apply(mat_die,1,sum)==0
  #df6<-df6[index_not_die,]
  #replot the graph
  #stot_slope_live<-ggplot(df6,aes(x=variable,y=log2,group=well_ID,colour=factor))+geom_point()+geom_line()+scale_color_manual(values = cbbPalette)+ylim(c(1,6))+labs(x="#days",y="log2 % area cells")+xlim(c(5,21))
  #s1_slope_live<-ggplot(df6[df6$factor=="01",],aes(x=variable,y=log2,group=well_ID,colour=factor))+geom_point()+geom_line()+scale_color_manual(values = cbbPalette[1])+ylim(c(1,6))+labs(x="#days",y="log2 % area cells")+xlim(c(5,21))
  #s4_slope_live<-ggplot(df6[df6$factor=="04",],aes(x=variable,y=log2,group=well_ID,colour=factor))+geom_point()+geom_line()+scale_color_manual(values = cbbPalette[2])+ylim(c(1,6))+labs(x="#days",y="log2 % area cells")+xlim(c(5,21))
  #s10_slope_live<-ggplot(df6[df6$factor=="10",],aes(x=variable,y=log2,group=well_ID,colour=factor))+geom_point()+geom_line()+scale_color_manual(values = cbbPalette[3])+ylim(c(1,6))+labs(x="#days",y="log2 % area cells")+xlim(c(5,21))
  #remake the analisys with the new df
  
  #mat_die2<-sapply(die,FUN = function(x){df_slope$well_ID==x})
  #index_not_die2<-apply(mat_die2,1,sum)==0
  #df_slope_live<-df_slope[index_not_die2,]
  #verify the removal of 15 wells
  #nrow(df_slope)-nrow(df_slope_live)
  ##################################################################
  #remake the analysis
  #make ANOVA over factor
  #anova2<-aov(slope~factor,data = df_slope_live[2:nrow(df_slope_live),])
  #summary(anova2)
  #Tukey
  #TukeyHSD(anova2)
  #basic table of the results
  #table2<-as.data.frame(as.list(aggregate(slope~factor,data = df_slope_live[2:nrow(df_slope_live),],FUN =function(x) c("slope"=mean(x),"sd"=sd(x)))))
  #colnames(table2)<-c("factor","slope","sd")
  #summary table of the parameters, define the df for each group
  #v2<-as.character(table2$factor)
  #table2$df<-sapply(v2,FUN = function(x){
    #length(unique(df_slope_live$well_ID[df_slope_live$factor==x]))-1
  #})
  #summary table of the parameters, define the critical value of the t statistic for each df (95% two tails)
  #table2$crit_value<-abs(qt(0.025,table2$df))
  #summary table of the parameters, define the se
  #table2$se<-table2$sd/sqrt(table2$df+1)
  #summary table of the parameters, define the 95% CI
  #table2$lower<-table2$slope-(table2$crit_value*table2$se)
  #table2$upper<-table2$slope+(table2$crit_value*table2$se)
  #write.csv(table2,paste0(getwd(),"/","table_mean_CI_alive_only.csv"),row.names = FALSE)
  
  #produce the boxplot and arrange the images
  #box_live<-ggplot(df_slope_live[2:nrow(df_slope_live),],aes(factor,slope))+geom_boxplot()
  #grid.arrange(stot_slope_live,s1_slope_live,s4_slope_live,s10_slope_live,box_live,layout_matrix=rbind(c(1,2,5),